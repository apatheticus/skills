...
Name the shape as well as the subject. The runtime/container view is an
`architecture` diagram by default and the request lifecycle a `sequence`, and both
defaults yield to what the code actually shows. A runtime that is one process with no
tiers to separate has no components to connect between — `layers` or `nested` is the
honest type there. A lifecycle that turns on a decision rather than passing messages
between actors is a `flowchart`, and one that hands work between owners is a
`swimlane`. The evidence that justifies the swap is the evidence the current-state
rule already demands: the entry points you traced, the configs that define the
boundaries, the call path you followed. If you can't point at the thing that makes it
a sequence, it isn't one. The routing table and the don't-draw test are in
[diagrams.md](diagrams.md); the chosen slug goes in the plan table, in `viz.json` as
...
SVG in the frozen design system** or **plain Mermaid** — not animated. A static SVG is
still a diagram and declares its type the same way: the layer map is usually a `tree`
or `nested`, the data model an `er`. Plain Mermaid declares nothing, because there is
no asset root to carry the attribute. Ground every
