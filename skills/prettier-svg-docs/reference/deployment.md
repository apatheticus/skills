...
The expected type follows that same branch, because the two flagships are different
pictures. A **promotion path** is a `process` — ordered environments with a gate on
each handoff — and it becomes a `flowchart` where the gates genuinely branch (a failed
check routes the change back rather than forward), or a `swimlane` where the story is
which team owns which step. **Deploy ordering**, the first flagship on a
single-environment repo, is a `sequence` when the point is which system acts when — CI,
the platform, the schema step, the container reporting healthy — and a `process` when
it is one pipeline with no second actor. The **rollback decision path** is a
`flowchart`; a table of failure classes resolving to roll back or fix forward is
decision logic and nothing else fits it. Each of these is a default, and what
overrides it is the config: a workflow with one job and no conditional has no branch
to draw, and a platform that promotes by retagging an image is not an exchange of
messages between actors. Picking a type that the config doesn't support is the same
failure as inventing an environment tier. Routing and the don't-draw test are in
[diagrams.md](diagrams.md); the slug goes in the plan table, in `viz.json` as
`diagram_type`, and on the asset root as `data-diagram`.
...
sequence — are **static SVG in the frozen design system** or **plain Mermaid**. Static
does not mean untyped: the topology is an `architecture`, the recovery runbook a
`sequence`, and each declares its slug exactly as a flagship does. Ground
