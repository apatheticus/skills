...
The PR lifecycle is a `flowchart` by default: it turns on whether checks pass, which
is decision logic. It becomes a `sequence` when the exchange between contributor,
reviewer and CI is what the reader needs, and a `swimlane` when the handoffs between
those three are the point rather than the branch. The branching model is the one
visual here with no clean type — the 27 in [diagrams.md](diagrams.md) carry no
git-graph grammar — so the honest answer is to leave it as the Mermaid `gitGraph` it
already is. Drawn as an SVG instead, take `process` for a branch → integration → main
promotion, or `timeline` when commit order is what the reader traces. Evidence for a
different choice comes from the git history the truth-sources section already sends
you to: a project that squash-merges through one integration branch has no second lane
to draw, and a review that never bounces back has no branch. The slug goes in the plan
table, in `viz.json` as `diagram_type`, and on the asset root as `data-diagram`.
