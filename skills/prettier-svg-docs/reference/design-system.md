...
visuals in that project — animated and static SVG alike. Freeze it once, early, then
...
#### Diagram-grammar roles map onto this palette; they never extend it

`reference/diagram-grammar.md` §2 names seven roles. They are **not** new palette
entries and must not be added to the table above — doing so would move `design_hash`
for every project and force a re-author of every visual in the repo, for no visual
gain. They resolve *through* whatever this project's palette happens to declare:

| Grammar role | Default binding | If the project has no such role |
| --- | --- | --- |
| `paper` | `background` | the lightest declared role, or the ground the style names |
| `surface` | `surface` | `background` |
| `ink` | `ink` | the highest-contrast declared role against `paper` |
| `muted` | `dim`, or `ink` at 0.65 | `ink` at 0.65 |
| `rule` | `dim` at 0.30, or `ink` at 0.12 | `ink` at 0.12 |
| `accent` | `accent-primary` | the one accent the project declares |
| `link` | `accent-secondary` | `accent`, and then only one of the two is used per board |

Two consequences of "map, don't extend" worth stating, because both look like bugs:

- **A six-role palette collapses several grammar roles onto one hex.** That is
  correct — the grammar distinguishes `muted` from `rule` so the *drawing* can, and a
  project that declares one grey gets one grey. It does not license inventing a
  second.
- **`link` folds into `accent` on a single-accent palette, and then the focal budget
  binds harder.** If external calls take the accent, they compete with the one or two
  focal nodes. Pick one — either external calls are the editorial point of the board,
  or the focal nodes are.

Record the bindings you chose in the mapping table above, next to the product-token
row if there is one. A later run reading only this file must be able to draw the same
board.
