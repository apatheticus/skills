...
- **No geometry, on a diagram.** On any visual whose root carries `data-diagram`, no
  animation may mutate `d`, `x`, `y`, `cx`, `cy`, `r`, `width`, `height`, `viewBox`, or
  a `translate`/`scale` transform on a `[data-node]` or `[data-edge]` — nor semantic
  text. The reason is mechanical rather than aesthetic: every geometry check in the
  `diagram` class measures the committed coordinates, so a board whose nodes drift
  passes its gate in one frame and fails in another. `svg_check.py` errors on it.
  A flow token travelling a *fixed* path is not geometry and stays legal.
...
**The travelling element carries no `cx`/`cy`, and that is not a stylistic choice.**
`<animateMotion>` *composes* with the element's own position rather than replacing it,
so a token written `<circle cx="296" cy="152">` on a path starting at `(296,152)`
renders at `(592,304)` — off the wire, often off the board. The snippet above omits
them for that reason. Nothing catches this: the file parses, gates clean, and is simply
wrong in a way only a render shows.
