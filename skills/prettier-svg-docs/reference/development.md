...
Budget: **1–2 flagship animated SVG diagrams** for this doc. Spend them where motion
...
Expected shapes, as defaults rather than mandates: the dev loop is a `process` — an
ordered run of stages, each handing off to the next — and the environment topology an
`architecture`. Two things push it elsewhere. A quick start that genuinely branches
(the build fails, you fix, you rerun) is a `flowchart`, and a build/seed/reset
workflow where several services take turns is a `sequence`. The evidence is the same
manifest and config the honesty rule already binds you to: if no script in the
manifest branches, the diagram has no branch to draw, and a loop drawn with a decision
diamond the commands don't have is the same fabrication as a `lint` script that
doesn't exist. Routing and the don't-draw test are in [diagrams.md](diagrams.md); the
slug goes in the plan table, in `viz.json` as `diagram_type`, and on the asset root as
`data-diagram`.
