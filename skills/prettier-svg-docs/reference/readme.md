...
**The hero has no diagram type, and giving it one is a mistake with a mechanical
cost.** It carries the project's idea, not a layout grammar, so it carries no
`data-diagram` and no `diagram_type` in `viz.json`, and the diagram gate never fires
on it — an invented slug would turn on a checker that has no nodes, connectors or
zones to measure. `tier: "animated-hero"` is what records that the absence is
deliberate rather than an oversight; a decorative banner takes `tier: "banner"` for
the same reason.

The body diagrams do carry types. The defaults: an architecture summary is
`architecture`, a key flow a `process` — or a `sequence` where the ordering between
actors is the point — and a data shape an `er`. A repo whose "key flow" is a decision
the reader has to follow is a `flowchart` instead, and one whose architecture summary
is really three stacked tiers is `layers`. What settles it is the same code the
summary is drawn from, since a README diagram is a compression of ARCHITECTURE and
inherits its grounding. Pick from [diagrams.md](diagrams.md), then write the slug into
the plan table, into `viz.json` as `diagram_type`, and onto the asset root as
`data-diagram`.
