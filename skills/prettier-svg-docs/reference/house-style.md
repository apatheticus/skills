...
This skill's signature: **key diagrams *are* seamless-loop animated SVG**,
hand-authored per the pipeline in `viz-production.md`, gated by
`scripts/svg_check.py`, embedded per `embedding.md`, and styled by the repo's
frozen design system (`design-system.md`) in the run's chosen style
(`styles.md`).

Nothing is rendered or converted. The `.svg` you author is the committed asset,
so there is no build step to go stale and no binary to inspect blind.
...
- **A diagram makes two machine-checkable statements about itself, and the drawing
  makes the first one.** `data-diagram="<slug>"` on the root declares the type, and
  the semantic attributes on the nodes, connectors, labels and zones declare what each
  mark is; together they are what turns on `svg_check.py`'s `diagram` class and give
  it something to count. The vocabulary is `diagram-grammar.md` and the type catalog
  is `diagrams.md` — neither is repeated here.
- **The collapsed `<details>` Mermaid block is the second statement**, and it
  immediately follows every animated diagram in a technical doc (ARCHITECTURE,
  DEVELOPMENT, DEPLOYMENT, CONTRIBUTING). It must parse and it must agree with the
  animation node for node. Its Mermaid type must also agree with the declared type: a
  `graph LR` under a visual carrying `data-diagram="sequence"` means one of the two is
  wrong, and until you know which, neither statement can be trusted. No checker owns
  that pairing, so it is a question to answer before shipping — does the Mermaid say
  the same thing the picture says, in the same shape? README embeds carry rich alt
  text instead — no Mermaid fallback there.
...
Animated SVG embeds animate on GitHub, GitLab, Codeberg and in VS Code's markdown
preview — a committed `.svg` is served as `image/svg+xml` with its CSS intact, and
the `<img>` context plays declarative animation. Renderers that rasterise SVG show
the t=0 frame, which is why the loop is steady-state and readable stopped; the
`<details>` Mermaid fallback covers anything that won't animate at all.
...
```json
{
  "$schema_note": "Config for the pretty-docs skills (pretty-hyper-docs, prettier-svg-docs, update-docs). Committed. No secrets.",
  "org": "Example Org, Inc.",
  "year_policy": "range",
  "first_year": 2026,
  "license": "Apache-2.0",
