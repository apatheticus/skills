...
**A diagram is built in a different order, because structure comes before material.**
The style says what the board is made of; the type says what it is shaped like, and
that decision has to be spent before a single coordinate is chosen:

1. **Choose the type** — `reference/diagrams.md`, then read exactly one
   `reference/types/<slug>.md`. Where behaviour rather than structure carries the
   meaning, pick a semantic pattern first (`reference/diagram-patterns.md`).
2. **Lay out on the 4-unit grid** — nodes placed, `viewBox` height including the
   legend strip, the primary axis picked and held.
3. **Route the connectors** — orthogonal elbows, fanned attach points, bridge hops,
   labels on open canvas with their 6–10 unit gap.
4. **Then the material** — the resolved style's fills, filters and texture.
5. **Then motion**, last, to a diagram that is already complete and correct at rest.

Doing material before layout is how a board ends up with a beautiful surface and a
diagonal connector, and the diagonal is the part a reader notices.
...
| **Diagram** | on a root carrying `data-diagram` only: the type resolves in `diagrams.json`; node / connector / focal / zone budgets; no diagonal connector; no label mask clipped by a later node or sitting within the gap floor of any connector; node geometry on the 4-unit grid; no animation that moves geometry; the legend below every node |
...
- `data-diagram="<slug>"` on the root turns the **diagram** class on, the same way
  `data-specimen` turns on `min_elements`. A hero, a banner or a style specimen has no
  type and carries no such attribute, so the class never fires on it. The semantic
  attributes it reads — `data-node`, `data-edge`, `data-label`, `data-focal`,
  `data-zone`, `data-legend` — go on the groups and paths they name; `<g>` is not
  drawn geometry, so the wrappers cost nothing against `min_elements`. The diagram
  class emits its own positive `NOTE`, and `0 diagonal, min label gap 8.0` is the line
  that says the connector grammar was actually applied rather than merely avoided.

  The alternative to these attributes is guessing — upstream has to infer that a node
  is a `<rect>` at least 60×40 and a label plate is 20–120 × 8–14, and concedes the
  thresholds need revisiting per type. Tagging the drawing makes every one of these
  checks exact.
