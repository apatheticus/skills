...
moving the falsehood, which is how this skill twice shipped a board whose accessible
name described the *previous* drawing.
...
- `producer`: always `prettier-svg-docs` for a visual this skill authored. The legacy
  ...
  `prettier-svg-docs` the next time the manifest is touched. Any *other* value means
...
- `diagram_type`: the slug from `scripts/diagrams.json` the visual was laid out as,
  present exactly when the asset's root carries `data-diagram`. The two must agree —
  `audit_visuals.py` reports a manifest and an asset that disagree, and one that
  declares a type the asset does not carry.

  Three states, and the third is the one that matters. A **slug** means the visual is
  a diagram of that type. **Absent** means nobody asked, and on a structural visual in
  a technical doc that is a finding. **Explicit `null`** means the question was asked
  and the honest answer was "none" — a hero, a banner, a contact sheet, a motif — and
  the audit accepts it and stops asking. Recording the negative answer is what keeps
  the gate from nagging about a visual that will never have a type, without letting an
  unasked question look the same as an answered one.
- `budget_cuts`: what the type's complexity budget forced out, as short phrases —
  `["merged the two cache nodes", "dropped the metrics sink to a legend entry"]`.
  Empty when nothing was cut. This is the same mechanism as `relaxed[]` one layer up:
  a softened gate is recorded so the softening is auditable, and a cut is recorded so
  the omission is. Without it, content dropped to fit nine nodes and content nobody
  noticed look identical to the next run.
...
| `FOREIGN` | `producer` is absent, or is neither `prettier-svg-docs` nor the legacy `more-pretty-docs` — the visual came from another skill and is a candidate for adoption | script |
| `UNTYPED` | The manifest's `diagram_type` and the asset's `data-diagram` disagree, or a structural visual in a technical doc records neither — see [diagrams.md](diagrams.md) | script |
