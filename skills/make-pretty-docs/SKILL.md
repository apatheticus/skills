...
description: Create and maintain a repository's standard documentation — README, ARCHITECTURE, DEVELOPMENT, CONTRIBUTING, CODE_OF_CONDUCT, SECURITY, SUPPORT, plus on-demand LICENSE, NOTICE, issue/PR templates, and CODEOWNERS — and beautify it with a per-repo design system and seamless-loop animated WebP visuals rendered via HyperFrames. Use this whenever the user wants beautiful, illustrated, animated, or visually polished project docs; wants a README with an animated hero or animated diagrams; wants doc visuals refreshed, re-rendered, or audited for staleness; or invokes /make-pretty-docs — even when they name only one file or none at all. For plain text-only doc maintenance without visuals, the sibling update-docs skill is the better fit.
---
...
via `scripts/audit_visuals.py`, works-without-images, budget + LICENSE/NOTICE
