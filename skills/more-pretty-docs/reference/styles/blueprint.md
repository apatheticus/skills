...
A drawing being made. Lines extend from their origin along their own length, dimension
arrows arrive after the geometry they measure, the grid stays still. `stroke-dashoffset`
does nearly all the work. Slow and mechanical — `6s` cycles, `linear` easing.
...
<style>
  ...
  .num   { fill: var(--dim); font-family: ui-monospace, SFMono-Regular, Menlo, monospace;
           font-size: 17px; }
  .draw  { stroke-dasharray: 600; animation: draw 6s linear infinite; }
  @keyframes draw {
