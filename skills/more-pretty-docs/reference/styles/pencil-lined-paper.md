...
Steady-state and small. A highlighter pass breathing between two opacities at `6s`;
a circled term's dash pattern creeping at `8s`; a packet travelling along a written
connector at `12s`, `ease-in-out`. **No draw-on entrance** — the note is already
written. Two motions at most; a page that keeps redrawing itself is a gimmick.
...
<style>
  ...
  .mark   { animation: mark 6s ease-in-out infinite; }
  @keyframes mark { 0%,100% { opacity: .34 } 50% { opacity: .62 } }
  @media (prefers-reduced-motion: reduce) { .mark { animation: none; opacity: .45 } }
</style>
