...
The hand at work, but **steady-state** — no draw-on entrance. A hatch block breathes
between two opacities, a construction arc's dash pattern creeps, a rubricated term
pulses once per long cycle. `9s` or `12s`, `ease-in-out`. One or two motions on the
whole board; a busy codex reads as a cartoon.
