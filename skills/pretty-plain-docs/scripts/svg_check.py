#!/usr/bin/env python3
"""Gate for hand-authored static SVG produced by the pretty-plain-docs skill.

Deterministic, python3 stdlib only. Checks, in order:

  structural  no <script>, no <foreignObject>, no remote href/@import;
              viewBox, <title>, <desc>, data-loop-s present — and the static
...
def parse_time(value: str) -> float | None:
    """'4s' / '500ms' / '.5s' / '4' -> seconds. Still needed for data-loop-s."""
...
class Stylesheet:
    ...
    def __init__(self, css: str) -> None:
        ...
        self.rules: list[tuple[str, dict]] = []   # (selector, decls)
        self.animated: dict[str, dict] = {}       # selector -> decls, has animation
        self.reduce_blocks = 0                    # leftover prefers-reduced-motion
        self.keyframes: dict[str, str] = {}
        self._load(strip_css_comments(css))

    def _add_rule(self, prelude: str, decls: dict) -> None:
        for sel in (s.strip() for s in prelude.split(",")):
            if not sel:
                continue
            self.rules.append((sel, decls))
            # `animation: none` counts. It is only ever written to cancel an
            # animation, so its presence means the file was authored to move.
...
def check_file(path: Path, catalog: dict, slug: str | None,
    ...
    # ---- structural, continued: the static contract
    #
    ...
    # static this skill produces stays legible to the animated siblings' tooling
    # instead of looking like a file that forgot to declare one. Here the only
    # permitted value is 0, and that declaration is the whole animation ban.
    ...
    # Scan every rule, not sheet.animated: that dict is keyed by selector, so a
    # reduce block's `animation: none` overwrites the base rule that names the
    # keyframes and every referenced block looks orphaned.
    unreferenced = set(sheet.keyframes)
    for _, decls in sheet.rules:
        for prop in ("animation", "animation-name"):
            for word in re.findall(r"[\w-]+", sheet.resolve(decls.get(prop, ""))):
                unreferenced.discard(word)

    if sheet.animated or smil or sheet.keyframes:
        parts = []
        if sheet.animated:
            parts.append(f"{len(sheet.animated)} animated selector(s) "
                         f"({', '.join(sorted(sheet.animated)[:4])})")
        if sheet.keyframes:
            parts.append(f"{len(sheet.keyframes)} @keyframes block(s) "
                         f"({', '.join(sorted(sheet.keyframes)[:4])})")
        if smil:
            parts.append("SMIL " + ", ".join(sorted({f"<{local(e.tag)}>" for e in smil})))
        f.error(label, "the file animates — " + "; ".join(parts) + ". Nothing in a "
                       "pretty-plain-docs visual moves: no @keyframes, no animation "
                       "declaration (`animation: none` included), no SMIL tag. Fold each "
                       "reduced-motion resting value onto its base rule and delete the "
                       "motion — viz-production.md → The animation ban")
    if unreferenced:
        # Called out separately because it survives a careless conversion: deleting
        # the `animation:` line and leaving the @keyframes behind reads as clean in a
        # grep for `animation:` and is dead weight in every byte count.
        f.error(label, f"@keyframes {', '.join(sorted(unreferenced))} is declared but "
                       "no rule references it — dead motion left behind by an "
                       "incomplete conversion")
    if sheet.reduce_blocks:
        f.warn(label, f"{sheet.reduce_blocks} @media (prefers-reduced-motion) block(s) "
                      "— nothing in a pretty-plain-docs visual moves, so the block has "
                      "nothing to reduce. Its resting declarations belong on the base "
                      "rules")
