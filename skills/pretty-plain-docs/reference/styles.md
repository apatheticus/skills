...
- **Style owns form; the product's brand tokens own the palette.** Shape, material,
  type treatment and composition come from the style. Colors come
...
mono everywhere, a character grid, whole-cell geometry → terminal. Rounded panes, a
sidebar, mixed proportional and mono type, Bézier connectors → ide-dark.
...
composition, and deliberately no logos, insignia, wordmarks, vessel or
...
- `viewBox`, `<title>`, `<desc>`, `data-loop-s` present, and `data-loop-s` is `0`
- Nothing animates — no `@keyframes`, no `animation:` declaration, no SMIL
