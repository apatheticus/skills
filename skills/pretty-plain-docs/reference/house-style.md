...
- [Diagrams: static SVG, Mermaid as source of truth](#diagrams-static-svg-mermaid-as-source-of-truth)
...
- **No individual feature announcements or bug-fix notes** ("now supports X",
  ...
  out.
- The same rules apply **inside every visual**: a version string or
  date baked into a visual goes stale silently. SVG text is at least greppable —
  unlike a rendered bitmap — but nothing prompts anyone to go looking, so the
...
## Diagrams: static SVG, Mermaid as source of truth

This skill's signature: **key diagrams *are* hand-authored static SVG**, produced
per the pipeline in `viz-production.md`, gated by `scripts/svg_check.py`, embedded
per `embedding.md`, and styled by the repo's frozen design system
(`design-system.md`) in the run's chosen style (`styles.md`).

Nothing is rendered or converted. The `.svg` you author is the committed asset,
so there is no build step to go stale and no binary to inspect blind. And because
nothing moves, the file survives every downstream path a doc actually takes — print,
PDF, a rasterising Markdown renderer, a docs-site build — with nothing lost.
...
- **Which visuals get a designed treatment is a budget decision** — see the
  visual-budget table in `SKILL.md`. Diagrams outside the budget are plain Mermaid.
- **Every structural visual is accompanied by a collapsed `<details>` block holding
  the equivalent Mermaid source**, placed after the closing `pd:viz` marker. That
  Mermaid is the machine-checkable statement of what the diagram depicts; it must
  parse and must agree with the SVG. This covers README body diagrams as well as the
  technical docs — a deliberate divergence from the animated siblings, argued in
  `readme.md` → The Mermaid rule. **The README header image is the one exemption**
  and carries rich alt text instead.
- **A chart is a special case with its own gate** — every plotted value must derive
  from a committed file. Read `charts.md` before drawing one; most requests for a
  chart are better served by a diagram or a table.
...
- Every internal link you write must resolve — the link-integrity gate checks this.
  Embedded asset paths (`docs/assets/*.svg`) count as internal links.
...
README can still carry its header image.
...
**A static `.svg` needs no renderer cooperation at all**, and that is this skill's
whole argument. There is no motion to play, no frame to be paused at, and no
capability to detect: any renderer that can draw an image draws the finished visual.
It is correct on GitHub, GitLab, Codeberg, VS Code's markdown preview, a
documentation-site build, a PDF export, a print stylesheet, and a reviewer's tool
that rasterises SVG — identically, because there is only ever one thing to show.
The `<details>` Mermaid source is a second, independent path to the same content for
anything that renders no images whatsoever.

The one compatibility rule that still matters is the *GitHub-safe hard rules* list in
`viz-production.md` — no `<script>`, no `<foreignObject>`, no remote references, an
opaque background of your own. Those are about what a sanitiser strips, not about
motion, and they hold here unchanged.
...
- `viz.style`: the resolved visual-style slug for this repo (see `styles.md`).
  ...
  `.prettydocs/prettydocs.md`, which is what visuals actually derive from.
...
4. **Cross-doc truth consistency.** Facts you emitted agree with the evidence pass
   ...
   header image shows must exist in ARCHITECTURE's text and the code).
...
6. **SVG validity.** Every visual authored this run passes
   `python3 scripts/svg_check.py <file.svg>` with **0 errors** — structure, the static
   contract (nothing animates), legibility floors, palette conformance, contrast,
   and the style's fidelity floor. Zero errors, not "only warnings". A `SOFTENED` line
   is a pass with a declared style relaxation and must appear in the report (see
   `styles.md`).
7. **Asset presence + size.** Every embedded `.svg` exists at its referenced path
   ...
   own declared floor, never past it).
...
9. **Works without images.** With every image stripped, each doc still makes its
   point: **every structural visual has its `<details>` Mermaid equivalent, which
...
- **Ground every visual.** A designed diagram asserts facts with more authority
  ...
  accurate partial diagram beats a complete fictional one. Pure decoration (motif,
  background texture) carries no facts and needs no grounding. **A chart is the
  sharpest form of this rule**: every plotted value must be recomputable from a
  committed file, which is why `charts.md` gates them separately.
