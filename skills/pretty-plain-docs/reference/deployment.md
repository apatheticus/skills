...
- an existing deployment doc **anywhere in the tree**, matched case-insensitively —
  `DEPLOYMENT.md`, `docs/deploy*`, `Docs/Deploy/DEPLOYMENT.md`, `deploy/README.md`.
  Then this is an UPDATE, not a CREATE

**Search for that last signal before concluding there is none, and search the whole
tree.** A repo with no platform config, no container, and a CI workflow that only
validates can still ship a full production deploy guide filed under `Docs/Deploy/`.
Checking the root alone reports `N/A — no deploy target` on a repo that has one, and
then risks authoring a **second** deployment doc beside the real one — the same defect
the design-system ladder forbids for `prettydocs.md`. When a deploy doc already exists
outside the conventional path, either update it in place or write the root
`DEPLOYMENT.md` as a pointer to it; never two.
...
Budget: **1–2 flagship static SVG diagrams** for this doc. Where the project has
...
a healthy container) and the **rollback decision path**. A diagram that wouldn't earn
a place in a printed operations runbook doesn't earn one here.

A deploy is genuinely a sequence, and this is the doc where a still must work hardest
to say so: **number every step**, and make the ordering readable without motion —
an operator following it mid-incident cannot afford to guess which box comes first.
This is also the doc most likely to be printed, PDF'd, or pasted into a ticket, which
is the case this skill exists for.

Every DEPLOYMENT diagram is **immediately followed by a collapsed
