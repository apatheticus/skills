...
designed header at most** — optional. If a header adds nothing, skip it; a plain
`# Support` heading is fine. The "How do I get help?" decision tree is a **Mermaid
`flowchart`**, or a designed SVG *with* its Mermaid source, and Mermaid alone is
usually the better answer: a routing tree is text with arrows, and the reader is
already frustrated. At most one `pd:viz` pair appears in this file. Any header is
centered like every other embed (`embedding.md` → Centering) and carries no version or
date (house-style → No volatile facts).
