...
rounded panes, mixed proportional and mono type, Bézier connectors. If
...
<rect x="520" y="86" width="10" height="24" fill="var(--ink)"/>

<rect x="320" y="492" width="840" height="48" fill="var(--pane)"/>
<text class="ui ok" x="348" y="522">0 problems</text>
```

The Bézier connector between panes is one `<path>`, drawn with the same curve the
editor itself would use — a shallow S, never a right angle.
...
above `10` on a pane, or a light canvas.
