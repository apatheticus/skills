...
A **soft scanline band** is the on-idiom overlay: one wide, low-opacity horizontal
band across the board, its edges feathered by a gradient rather than hard. One band
per board — two read as a printing fault rather than a finish.
...
The glint is clipped to the chrome shape, so the highlight ends exactly at the bevel
instead of bleeding past it — that hard stop is what makes the surface read as metal
...
band per board, a chrome gradient without the hard light band or without the dark
break at the base (it just looks grey), a flat stroke where the
