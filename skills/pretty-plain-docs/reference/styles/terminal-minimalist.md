...
form. This one is a TUI — radius `0`, a character grid, monospace everywhere.
`ide-dark` is the GUI: rounded panes, mixed proportional and mono type,
Bézier connectors. If your reference image has rounded corners and a
...
The cursor block and a stepped progress bar:
...
The bar is drawn in whole cells — eight filled of twelve, never a fractional cell —
so it reads as a terminal redraw rather than a rendered gauge.
...
Bézier connectors, ASCII/box-drawing characters standing in for real strokes,
colour used decoratively rather than semantically, status carried by colour
