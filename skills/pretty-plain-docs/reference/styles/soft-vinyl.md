...
Nothing moves here, but the reason the animated sibling caps its motion at a few percent
is a **material** fact, and it applies to a still just as much: a baked directional
gradient deforms *with* its form. Scale a slab non-uniformly — in an editor, or by
reusing one form at a second aspect ratio — and its shading scales too, which is wrong,
because the light did not move. The form turns to rubber somewhere past roughly 8%.

So a form is never stretched to fit. Re-emit it at the size you need, with the gradient
axis recomputed from the new geometry, which is what `userSpaceOnUse` exists to make
possible. This is the same failure that makes `objectBoundingBox` unusable for this
style, arriving by a different route.
