...
Budget: **1–2 flagship static SVG diagrams** for this doc. For CONTRIBUTING the
...
the project's tiers are non-trivial. Draw a diagram only if it would earn a place
in a printed engineering doc — a two-branch flow is a sentence, and drawing it
decorates rather than clarifies.

Every CONTRIBUTING diagram is **immediately followed by a collapsed
`<details><summary>Diagram source (Mermaid)</summary>` block** with the equivalent
Mermaid, which must parse (validate it) and **agree** with the SVG. Any remaining
diagram is **plain Mermaid**. Keep
