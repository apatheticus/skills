...
Budget: **1–2 flagship static SVG diagrams** for this doc. Spend them where a drawn
picture beats a paragraph — for DEVELOPMENT that's usually the **dev loop**
...
workflow with several stages can justify the second one. The test is the printed
engineering doc: if a diagram wouldn't earn a page there, it doesn't earn one here.

Ordering carries the meaning here, and a still has to state it rather than perform
it. Number the steps, or draw one unambiguous flow direction and label the arrows —
never rely on the reader inferring sequence from position.

Every DEVELOPMENT diagram is **immediately followed by a collapsed
`<details><summary>Diagram source (Mermaid)</summary>` block** with the equivalent
Mermaid, which must parse (validate it) and **agree** with the SVG step for
step. Any remaining diagram is **plain Mermaid**. Keep versions, dates, and
per-release notes out of the rendered pixels (house-style → No volatile facts).
