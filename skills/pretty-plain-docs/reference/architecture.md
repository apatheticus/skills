...
leave a `<!-- TODO -->` — never draw a plausible-but-fictional box-and-arrow (a
designed diagram asserts facts with more authority than prose — house-style →
Anti-fabrication).
...
Budget: **1–2 flagship static SVG diagrams** for this doc. Spend them on the
diagrams with the highest explanatory payoff — for ARCHITECTURE that's almost always
the **runtime/container view** and the **request lifecycle**. A diagram earns a
designed treatment only if it would earn a place in a printed engineering doc; it must
clarify a boundary or a flow, never decorate.

A request lifecycle is a sequence, and a still has to *state* the order rather than
perform it: number the hops, or commit to one flow direction and label every arrow.
An unnumbered ring of boxes is ambiguous on paper, and paper is this skill's target.

Every ARCHITECTURE diagram is **immediately followed by a collapsed
`<details><summary>Diagram source (Mermaid)</summary>` block** holding the equivalent
Mermaid. That Mermaid is the machine-checkable statement of what the diagram
depicts: it must parse (validate it — house-style → Diagrams) and it must **agree**
with the SVG, node for node. If the picture and the source disagree, the doc is
wrong.

All remaining diagrams (the layer map, the data model, secondary flows) are **plain
Mermaid**. Ground every
