...
| Header image | required (unless `--no-viz`) | the project's core idea |
...
| Architecture (summary + at most 1 diagram, links to ARCHITECTURE.md) | conditional | code |
...
- **Project structure.** A plain ```` ``` ```` code block directory tree (this file
  listing is the one allowed non-Mermaid, non-SVG "diagram"), annotated with one
...
- **One header image**, placed immediately below the badge cluster. It carries the
  project's core idea — the thing the project *does* — not a decorative banner. This
  is the flagship visual for the whole repo.
- **Up to 3 diagrams** in the body (architecture summary, a key flow, a data
  shape) — only where a drawn picture genuinely orients a newcomer faster than prose.
  Fewer is better; don't spend the budget just to fill it.

That's the ceiling: **header + at most 3 diagrams.** A `--brief` README still
gets its header image; `--no-viz` drops all of them and the doc must still read
cleanly.

Every README visual is a static SVG embedded through a `pd:viz` marker (format
...
**Every embed is centered**, header image and body diagrams alike, each in its own
`<div align="center">` wrapper inside its marker pair. The header image looks centered
...
### The Mermaid rule, and how it differs from the animated siblings

**Every structural visual in the README body — a diagram or a chart — carries a
collapsed `<details><summary>Diagram source (Mermaid)</summary>` block, exactly as the
technical docs do.** The block sits immediately *after* the closing `pd:viz` marker,
outside the pair. It must parse and it must agree with the SVG node for node.

**The header image is the one exemption.** It is a designed statement of what the
project is, not a structure with nodes and edges, so there is nothing for a Mermaid
graph to be equivalent *to*. Instead it carries rich `alt` text conveying the actual
meaning — the components, the direction of flow, the point being made — so the README
...
equivalent and a Mermaid block would understate the visual. A static diagram has no
such surplus — everything it says is structure, which is exactly what Mermaid encodes.
So here the machine-checkable source is both achievable and worth more: it can be
validated, diffed in a PR, and read by a tool, and it is the thing that catches a
diagram whose picture and facts have drifted apart.

Charts follow the same rule with a table as the permitted fallback where Mermaid
cannot carry the data — [charts.md](charts.md) → The Mermaid fallback.

Anything beyond the budget is plain Mermaid. Keep versions and dates **out** of every
rendered visual (house-style → No volatile facts) — a baked-in `v2.4.1` goes stale
silently and can't be greped.
...
​```mermaid
flowchart LR
  <a>["<real component>"] --> <b>["<real component>"]
​```
