...
**`glassmorphism` was derived, not chosen, and it is rendered light — which is the opposite of how the catalog draws it.** The derivation is not aesthetic preference: the report's own §1 hero is `sp-glass`, and `--surface-glass` / `--border-glass` are first-class SaaS Pro tokens, so the product already *is* a glass system. But SaaS Pro is light-only and says so, and `--navy-*` is a card surface that must not be promoted to a page ground — so the field is a light lavender page with brand-ramp blobs and the panes are near-white with dark ink. That inverts the style's most common failure mode (the spec warns white-on-glass lands at 1.68:1) instead of fighting it. Two divergences are deliberate and documented in B, and a later run must not "correct" them: the pane edge ramp runs `glass` → `brand` rather than white → white, because a white border on a near-white pane over a light page is invisible; and the §4 glass recipe's **inset top highlight is omitted**, because on a light pane there is no darker interior for it to separate from and it reads as a stray hairline.

**Three findings from authoring those two boards, none of which reading the specs would have produced.**
(1) **`pane` `#DBDCFB` is a palette role that is a *measurement*.** A pane is two `glass` layers at `0.60` and `0.45` — `0.78` coverage — so the ground under a label is `glass` composited over whichever blob is behind it, and the drift means the ratio is a *range*, not a number. The role is that composite over the lowest-luminance blob, and `data-bg="pane"` points every label at the worst case. Declaring the role as pure white would pass `--ink-500` at 4.84:1 when it actually renders at **3.60:1**. Heavier coverage is not the fix: at `0.92` the frost stops being visible and the pane is just a white rectangle.
...
The catalog `glassmorphism` spec has a **latent seam defect in its own specimen** that these boards deliberately do not copy: `docs/samples/glassmorphism.svg` animates with `alternate` at `12s` inside a `data-loop-s="12"` loop, which is **one** half-cycle, so `t=D` is the far end of the drift rather than the start. `svg_check.py` cannot catch it — it only proves the duration divides the loop — and the style's own prose says an odd number of `alternate` half-cycles breaks the seam. `reflect`'s visuals use symmetric `0% / 50% / 100%` keyframes instead, which returns to origin by arithmetic. Fixing the specimen is a separate change.
