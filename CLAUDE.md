...
- **`reflect` touches two design systems and they are constantly confused.** The HTML report the skill *generates* is styled in **Neumorphic Fresh**, read from the user's own copy at `/Users/luke/scratch/Styles/Neumorphic Fresh Design System/` — it is deliberately **not** bundled in the skill, so it stays current when the user updates it. `.prettydocs/prettydocs.md` governs `reflect`'s own README visuals; its palette was derived from the old bundled SaaS Pro copy and is now frozen inline. Check which one you are in before editing. Neumorphic Fresh has real light **and** dark themes — the report ships a working `data-theme` toggle; never hard-code a hex.
